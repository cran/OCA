...
#'}
#' Where \eqn{$K$}{K} is the aggregate capital to be allocated, \eqn{CTE_{p}[S]}{CTE_p[S] } is the
#' Conditional Tail Expectation of the aggregate loss at level \emph{p}, \eqn{$X_{i}$}{X_i} is the
#' individual loss, \emph{S} is the aggregate loss and \eqn{F_{X}^{-1}(p)}{ F_X^-1(p)} is the quantile
#' function of \eqn{$X$}{X} at level  \emph{p}.
#'
#' @return
